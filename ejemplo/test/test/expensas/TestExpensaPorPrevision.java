...
public class TestExpensaPorPrevision extends TestCase {
	...
	public List<GastoDTO> obtenerGastosOrdinariosPorEdificioYPeriodo(int idEdificio,Periodo periodo){
		...
		Date dia = periodo.toDate();
		System.out.println(dia);
		Periodo periodoAnterior = periodo.obtenerPeriodoAnterior();
		
		
		Query query = session.createQuery("select g from GastoDTO g,GastoRealDTO gr where " +
							"gr=g and gr.fechaPago between :fechaInicio and :fechaFin and gr.edificio =:idEdificio" +
							" and g.tipoGasto.tipo <> 'EXT' ");
		
		query.setString("fechaInicio", periodoAnterior.obtenerFechaInicio());
		query.setString("fechaFin", periodoAnterior.obtenerFechaFin());
		query.setInteger("idEdificio", idEdificio);							
					
		List<GastoDTO> gastos = query.list();
					
		query = session.createQuery("select g from GastoDTO g,GastoPrevisionDTO gp where " +
						"gp=g and gp.anio =:anio and gp.mes =:mes and g.tipoGasto.tipo <> 'EXT' ");
		
		query.setInteger("anio", periodo.getAnio());
		query.setInteger("mes", periodo.getMes());
		
		gastos.addAll(query.list());
		
		return gastos; 
	}
	
	public void testObtenerGastosPrevisionesPorEdificio(){
		Periodo periodo = new Periodo();
		periodo.setAnio(2010);
		periodo.setMes(11);
		List<GastoDTO> gastosYPrevisiones= obtenerGastosOrdinariosPorEdificioYPeriodo(1, periodo);
		for (GastoDTO gp : gastosYPrevisiones) {
			System.out.println(gp.getTipoGasto().getCodigo()+" - "+gp.getDetalle()+ " : "+ gp.getMonto());
		}
		
		//TODO Se recuperaron los gastos ahora a restar!!
	}
	...
	public HashMap<TipoGastoDTO, List<GastoDTO>> obtenerGastosPorEdificioAgrupadoPorTipo(int idEdificio){
		Periodo periodoLiquidacion = new Periodo();
		periodoLiquidacion.setAnio(2010);
		periodoLiquidacion.setMes(11);
		
		List<GastoDTO> gastos = obtenerGastosOrdinariosPorEdificioYPeriodo(idEdificio, periodoLiquidacion);
