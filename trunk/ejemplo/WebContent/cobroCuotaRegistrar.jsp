...
function validar(thisform) {
	...
	if (valido==true) {
		document.regisCuotaCobro.submit();
	}
}
</script>
<div class="contenido">
			 			...
			 			<tr>
				 			...
				 			<td><label for="fechaPago">Fecha de Pago:&nbsp;</label>  </td>
					  		<td><input style="display: none;" type="text" id="fecha" name="fecha" size="15"/>
									...
									&nbsp;<a href="JavaScript:doNothing()" onclick="allowPrevious=true;setDateField(document.regisCuotaCobro.dia,document.regisCuotaCobro.mes,document.regisCuotaCobro.anio);top.newWin = window.open('calendario.jsp','cal','WIDTH=200,HEIGHT=160,TOP=200,LEFT=300')" onMouseOver="javascript: window.status = 'Abrir calendario'; return true;" onMouseOut="window.status=' '; return true;" ><img src="images/calendario.gif" ></img></a>
				 			</td>
				 		</tr>
			  		</table>		
			</fieldset>
			<input class="btn" type="button" value="Aceptar" onclick="validar()" />
			<input type="text" style="display: none;" id="cuota_id" name="cuota_id" value=<%=cuota.getId() %> size="15"/>		  	
